...
open denotes open parentheses count , similary close denotes close parentheses count
...
// This function is used to generate all well-forrmed combination of parentheses
...
void generateParenthesis(int open, int close, string op, vector<string>&ans)
{
	...
	if (open == 0 && close == 0)
	{
		ans.push_back(op);  // push the output valid string combination to the answer vector and return

		return;
	}

	if (open != 0)     // open bracket will always be one of the choice , given only the condition that open != 0
	{
		string op1 = op;    

		op1.push_back('(');  // output1 string (like we can see in recursive tree , it is the output string of when  we select '(')
  ...
  // As the closing parenthesis cannot occur before the open parenthesis for generating valid combination. 
	...
	if (close != 0 && close > open) 
	{
		...
		op2.push_back(')');  // output2 string (like we can see in recursive tree , it is the output string of when  we select ')')
...
int main()
{
	...
	vector<string>ans;  // this is our output vector

	int open = n;  // this represents the open parentheses count.

	int close = n;  // this represents the close parentheses count

	string op = "";  // this is the output string , which will be out valid parentheses combination string , initially it is empty

	generateParenthesis(open, close, op, ans); // let's call the function

	for (int i = 0 ; i < ans.size(); i++)  // printing output vector
	{
		cout << ans[i] << " ";
	}
	cout << endl;
